...
def clear_page(web):
    if web.find_elements_by_class_name('btn-primary'):
        ...
        pass
    sleep(1)
    if web.find_element_by_xpath("//span/a[@title='关闭']"):
...
# 学习课程名称
def subject_name(sub_name, sub_bool):
    if sub_bool == 'n' or sub_bool == 'N':
        sub_name = input('请输入学习课程:')
    return sub_name
...
if __name__ == '__main__':
    ...
    subjects = []  # 科目列表
    subject_bool = input('\n默认学习课程为"2019年一季度机关中台",如需修改请按"N":')
    subjects_name = '2019年一季度机关中台'
    subject_name(subjects_name, subject_bool)
