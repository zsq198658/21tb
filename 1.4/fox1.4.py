...
if __name__ == '__main__':
    ...
    while last_group == False:
        try:
            browser.find_element_by_xpath('//button[@class="track-node drag-node track-node-last track-node-current"]')
        except:
            # 不是最后一阶段
            print('不是最后课程')
            # 如果有'进入下一阶段',点击按钮
            try:
                browser.find_element_by_xpath('//button[@class="certificate-btn self-promotion track-blue"]')
            except:
                # 获取课程列表
                ...
                browser.execute_script("arguments[0].scrollIntoView();", next_button)  # 使next_button能够点击到
                next_button.click()
                print('进入下一阶段')
                # 重开课程列表页
                find_learn_list_page(browser, subjects_name)
