...
# 分阶段学课
def for_phasing(web, for_subjects, for_subjects_name):
    last_group = False
    # 是不是最后一阶段
    while not last_group:
        try:
            web.find_element_by_xpath('//div[@class="track-node drag-node track-node-last'
                                      ' track-node-current track-node-passed"]')
        except:
            # 不是最后一阶段
            print('不是最后课程')
            # 是否有'进入下一阶段',点击按钮
            try:
                web.find_element_by_xpath('//button[@class="certificate-btn self-promotion track-blue"]')
            except:
                # 没有'进入下一阶段'按钮,则获取课程列表
                get_subjects_list(web, for_subjects)
                # 学习课程
                for i in range(0, len(for_subjects)):
                    web.execute_script("arguments[0].scrollIntoView();", for_subjects[i])  # 使subjects[i]能够点击到
                    for_subjects[i].click()
                    print('\n当前学习课程：' + for_subjects[i].text)
                    web.switch_to.window(browser.window_handles[1])
                    virtual_learn(web, for_subjects[i])
                    web.switch_to.window(web.window_handles[0])
                    web.find_element_by_class_name('//a[@class="tbc-panel-btn-close"').click()
                find_learn_list_page(web, for_subjects_name)
            else:
                # 有'进入下一阶段'按钮
                next_button = web.find_element_by_xpath(
                    '//button[@class="certificate-btn self-promotion track-blue"]')
                web.execute_script("arguments[0].scrollIntoView();", next_button)  # 使next_button能够点击到
                next_button.click()
                print('进入下一阶段')
                # 重开课程列表页
                web.find_element_by_class_name('//a[@class="tbc-panel-btn-close"').click()
...
if __name__ == '__main__':
    ...
    # 分阶段学习课程
    for_phasing(browser, subjects, subjects_name)
